...
import { Validation } from '@ducatus/crypto-wallet-core';
...
export class WalletService {
   ...
   * @param {string} opts.coin[='btc'] - The coin for this wallet (btc, bch, eth, doge, ltc, duc, ducx).
   ...
   * @param {string} opts.coin[='btc'] - The expected coin for this wallet (btc, bch, eth, doge, ltc, duc, ducx).
    ...
    bc.estimateFee(points, (err, result) => {
      ...
      const levels = _.fromPairs(
        _.map(points, p => {
          ...
          // NOTE: ONLY BTC/BCH/DOGE/LTC/DUC/DUCX expect feePerKb to be Bitcoin amounts
          // others... expect wei.
   ...
   * @param {Object} opts
   * @param {String} opts.coin - The coin requested (btc, bch, eth, xrp, ltc, duc, ducx).
   * @param {String} [opts.code] - Currency ISO code (e.g: USD, EUR, ARS).
   * @param {Date} [opts.ts] - A timestamp to base the rate on (default Date.now()).
   * @param {String} [opts.provider] - A provider of exchange rates (default 'BitPay').
   * @returns {Array} rates - The exchange rate.
