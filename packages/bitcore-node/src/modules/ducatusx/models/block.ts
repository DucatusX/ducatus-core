...
import { IDucxBlock, IDucxTransaction } from '../types';
import { DucxTransactionStorage } from './transaction';

@LoggifyClass
export class DucxBlockModel extends BaseBlock<IDucxBlock> {
  ...
  async processBlock(params: {
    ...
    await DucxTransactionStorage.batchImport({
      txs: transactions,
  ...
  async getBlockOp(params: { block: IDucxBlock; chain: string; network: string }) {
    const { block, chain, network } = params;
  ...
  async handleReorg(params: { block: IBlock; chain: string; network: string }): Promise<boolean> {
    ...
    const reorgOps = [
      ...
      DucxTransactionStorage.collection.deleteMany({ chain, network, blockHeight: { $gte: localTip.height } })
    ];
  ...
  _apiTransform(block: Partial<MongoBound<IDucxBlock>>, options?: TransformOptions): any {
    const transform = {
      _id: block._id,
      chain: block.chain,
...
export let DucxBlockStorage = new DucxBlockModel();
