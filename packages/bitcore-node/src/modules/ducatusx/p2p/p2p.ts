...
import { DucxBlockModel, DucxBlockStorage } from '../models/block';
import { DucxTransactionModel, DucxTransactionStorage } from '../models/transaction';
import { IDucxBlock, IDucxTransaction, ParityBlock, ParityTransaction } from '../types';
import { ParityRPC } from './parityRpc';

export class DucxP2pWorker extends BaseP2PWorker<IDucxBlock> {
  protected chainConfig: any;
  protected syncing: boolean;
  protected initialSyncComplete: boolean;
  protected blockModel: DucxBlockModel;
  protected txModel: DucxTransactionModel;
  protected txSubscription: any;
  protected blockSubscription: any;
  protected rpc?: ParityRPC;
  protected provider: DUCXStateProvider;
  protected web3?: Web3;
  ...
  constructor({ chain, network, chainConfig, blockModel = DucxBlockStorage, txModel = DucxTransactionStorage }) {
    ...
    this.provider = new DUCXStateProvider(chain);
  ...
  async processBlock(block: IDucxBlock, transactions: IDucxTransaction[]): Promise<any> {
  ...
  async convertBlock(block: ParityBlock) {
    ...
    const convertedBlock: IDucxBlock = {
      chain: this.chain,
      network: this.network,
      height,
      hash,
  ...
  convertTx(tx: Partial<ParityTransaction>, block?: IDucxBlock): IDucxTransaction {
    if (!block) {
      const txid = tx.hash || '';
      const to = tx.to || '';
      const from = tx.from || '';
      ...
      const convertedTx: IDucxTransaction = {
        chain: this.chain,
        network: this.network,
