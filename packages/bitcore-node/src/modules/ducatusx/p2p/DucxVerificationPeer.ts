...
import { DucxP2pWorker } from './p2p';

export class DucxVerificationPeer extends DucxP2pWorker implements IVerificationPeer {
  prevBlockNum = 0;
  prevHash = '';
  nextBlockHash = '';
  deepScan = false;
  ...
  async resync(start: number, end: number) {
    ...
    while (currentHeight <= end) {
      ...
      const nextBlock = await DucxBlockStorage.collection.findOne({ chain, network, previousBlockHash: block.hash });
      if (nextBlock) {
